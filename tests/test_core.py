...
def make_jpeg_pattern(seed: int = 0) -> bytes:
    """Создаёт JPEG с уникальным шумовым паттерном — разные phash."""
    import random
...
class TestImageProcessor:
    ...
    def test_different_images_not_duplicate(self):
        ...
        jpeg1 = make_jpeg_pattern(seed=1)
        jpeg2 = make_jpeg_pattern(seed=999)
    ...
    def test_is_duplicate_same_hash(self):
        ...
        jpeg = make_jpeg_pattern(seed=42)
        img = Image.open(io.BytesIO(jpeg))
        phash = self.compute_phash(img)
        assert self.is_duplicate(phash, phash) == True

    def test_is_duplicate_different_hash(self):
        """Совершенно разные хеши — не дубль."""
        img1 = Image.open(io.BytesIO(make_jpeg_pattern(seed=1)))
        img2 = Image.open(io.BytesIO(make_jpeg_pattern(seed=999)))
        h1 = self.compute_phash(img1)
        h2 = self.compute_phash(img2)
        assert self.is_duplicate(h1, h2) == False
