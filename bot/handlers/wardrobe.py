...
from services.selfie_analysis import (
    _analyze_selfie_colortype, _build_colortype_card, _save_colortype_to_user,
    _COLORTYPE_NAMES_RU, _send_style_passport, _build_passport_palette, _PASSPORT_HEX,
)
...
async def _analyze_and_save(
    ...
    *,
    ...
    # Resolve overlapping bboxes and refine by color
    from services.vision import _resolve_bbox_overlaps, _refine_bbox_by_color
    items_data = _resolve_bbox_overlaps(items_data)
    items_data = _refine_bbox_by_color(photo_bytes, items_data)
...
async def _generate_outfit_for_user(message, user, context, exclude_ids: set | None = None, silent_status: bool = False) -> None:
    ...
    _typing_task = _track_task(_keep_typing(), name="typing_indicator")
