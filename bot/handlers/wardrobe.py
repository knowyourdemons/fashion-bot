...
def _warm_outfit_comment(score: float, child_name: str = None,
    ...
    if score >= 8.5:
        options = [
            ...
            f"Прекрасный выбор для {name_dat}! {child_name or 'Ты'} будет выглядеть замечательно",
            f"Образ для {name_dat} на все сто! Цвета отлично играют вместе",
        ]
    elif score >= 7.0:
        options = [
            f"Хороший образ для {name_dat} — и тепло, и красиво!",
            f"Достойный выбор для {name_dat} на сегодня. Всё по погоде!",
            f"Симпатичный образ для {name_dat}! Добавь яркий аксессуар — будет ещё лучше",
        ]
    elif score >= 5.0:
        options = [
            f"Удобный образ для {name_dat} на каждый день",
            f"Практичный выбор для {name_dat}! Добавь пару вещей — комбинаций станет больше",
        ]
    else:
        options = [
            f"Собрала для {name_dat} из того что есть. Добавь ещё вещей — образы станут интереснее!",
        ]
